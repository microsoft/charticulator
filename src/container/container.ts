...
import { Color, Dataset, deepClone, Expression, getById, getByName, getField, Graphics, initialize, Prototypes, Scale, setField, Solver, Specification } from "../core";
                    ...
                    for (const inference of template.inference[id]) {
                        switch (inference.type) {
                            case "axis": {
                                ...
                                const slot = getByName(this.template.dataSlots, axis.slotName);
                                if (expression == null || slot == null) { continue; }
                                const original = this.getProperty(object, axis.property, axis.fields) as Specification.Types.AxisDataBinding;
                                original.expression = expression;
                                // Infer scale domain or mapping
                                const columnVector = getExpressionVector(this.tableAssignment[slot.table], expression);
                            ...
                            case "scale": {
                                ...
                                const slot = getByName(this.template.dataSlots, scale.slotName);
                                    ...
                                    case "categorical": {
                                        ...
                                        switch (scale.rangeType) {
                                            ...
                                            case "color": {
                                                ...
                                                const palette = getDefaultColorPalette(s.domain.size);
                                                s.domain.forEach((index, key) => {
                            ...
                            case "order": {
                                ...
                                const slot = getByName(this.template.dataSlots, order.slotName);
