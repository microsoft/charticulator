...
export function parseDate(str: string) {
    ...
    let m;
    // YYYY-MM-DD
    m = str.match(/^([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})( +([0-9]{1,2})\:([0-9]{1,2})(\:([0-9]{1,2}))?)?$/);
    if (m) {
        if (m[5] != null) {
            if (m[8] != null) {
                return new Date(+m[1], +m[2] - 1, +m[3], +m[5], +m[6], 0, 0).getTime();
            } else {
                return new Date(+m[1], +m[2] - 1, +m[3], +m[5], +m[6], +m[8], 0).getTime();
            }
        } else {
...
export let dataTypes: { [name: string]: DataTypeDescription } = {
    boolean: {
        test: (x: string) => x.toLowerCase() === "true" || x.toLowerCase() === "false",
        convert: (x: string) => x.toLowerCase() === "true" ? true : false
    },
    integer: {
        test: (x: string) => (!isNaN(+x.replace(/\,/g, ""))) && (+x.replace(/\,/g, "")) === ~~(+x.replace(/\,/g, "")),
    ...
    date: { // date is represented as unix timestamp, in milliseconds
