...
import { AttributeDescription, BoundingBox, BuildConstraintsContext, Controls, DropZones, Handles, LinkAnchor, ObjectClasses, ObjectClassMetadata, SnappingGuides, TemplateParameters } from "../common";
...
export class DataAxis extends MarkClass {
        ...
        if (props.axis) {
            if (props.axis.type == "numerical") {
                for (let i = 0; i < props.dataExpressions.length; i++) {
                    const expr = context.getExpressionValue(props.dataExpressions[i], context.rowContext) as number;
                    const t = (expr - props.axis.domainMin) / (props.axis.domainMax - props.axis.domainMin);
    ...
    public getSnappingGuides(): SnappingGuides.Description[] {
        ...
        if (attrs.x1 != attrs.x2) {
            for (let i = 0; i < this.object.properties.dataExpressions.length; i++) {
                ...
                guides.push({ type: "x", value: attrs[attr] as number, attribute: attr } as SnappingGuides.Axis);
            }
        }
        if (attrs.y1 != attrs.y2) {
            for (let i = 0; i < this.object.properties.dataExpressions.length; i++) {
                const attr = `anchorY${i}`;
                guides.push({ type: "y", value: attrs[attr] as number, attribute: attr } as SnappingGuides.Axis);
            }
        }
        for (let i = 0; i < this.object.properties.dataExpressions.length; i++) {
            const attrX = `anchorX${i}`;
            const attrY = `anchorY${i}`;
            guides.push({ type: "label", x: attrs[attrX] as number, y: attrs[attrY] as number, text: this.object.properties.dataExpressions[i] } as SnappingGuides.Label);
        }
        return guides;
    }
    ...
    public getAttributePanelWidgets(manager: Controls.WidgetManager): Controls.Widget[] {
        const props = this.object.properties;
        const axisWidgets = buildAxisWidgets(props.axis, "axis", manager, "Data Axis");
        return [
            ...axisWidgets
        ];
    }
