...
export interface CategoricalScaleProperties<ValueType extends Specification.AttributeValue> extends Specification.AttributeMap {
...
export interface CategoricalScaleNumberAttributes extends CategoricalScaleAttributes {
    rangeScale?: number;
}
...
class CategoricalScaleNumber extends ScaleClass {
    ...
    public readonly object: { properties: CategoricalScaleProperties<number> } & Specification.Scale;
    public readonly state: CategoricalScaleNumberState;

    public attributeNames: string[] = ["rangeScale"];
    public attributes: { [name: string]: AttributeDescription } = {
        rangeScale: { name: "rangeScale", type: "number", displayName: "Scale", category: "scale-range", strength: VariableStrength.MEDIUM }
    };
    ...
    public inferParameters(column: Specification.DataValue[], hints: DataMappingHints = {}): void {
        ...
        s.domain.forEach((v, d) => {
            props.mapping[d] = (v / (s.domain.size - 1)) * (range[1] - range[0]) + range[0];
        });
...
class CategoricalScaleColor extends ScaleClass {
    ...
    public readonly object: { properties: CategoricalScaleProperties<Color> } & Specification.Scale;
    ...
    public getAttributePanelWidgets(manager: Controls.WidgetManager): Controls.Widget[] {
        ...
        items.push(manager.table(
            keys.map(key => [
                manager.text(key, "right"),
                manager.inputColor({ property: "mapping", field: key, noComputeLayout: true })
            ])
        ));
        return [
...
class CategoricalScaleString extends ScaleClass {
    ...
    public readonly object: { properties: CategoricalScaleProperties<string> } & Specification.Scale;
...
class CategoricalScaleBoolean extends ScaleClass {
    ...
    public readonly object: { properties: CategoricalScaleProperties<boolean> } & Specification.Scale;

    public attributeNames: string[] = ["mapping"];
    public attributes: { [name: string]: AttributeDescription } = {
        mapping: { name: "mapping", type: "map<string,boolean>", displayName: "Mapping", category: "scale-domain", solverExclude: true }
    };
    ...
    public getAttributePanelWidgets(manager: Controls.WidgetManager): Controls.Widget[] {
        ...
        return [
            ...
            manager.horizontal([0, 0],
                manager.setButton({ property: "mapping" }, mappingALL, null, "Select All"),
