...
export function loadDataFromURL(url: string, contentType: string = "text", timeout: number = 10): Promise<any> {
    return fetch(url).then((response) => {
        if (response.ok && response.status == 200) {
            if (contentType == "text") return response.text();
            if (contentType == "json") return response.json();
            if (contentType == "arraybuffer") return response.arrayBuffer();
