...
export class Dispatcher<ActionType> {
    ...
    private registeredItems = new Map<string, { order: [number, number], stage: number, callback: (action: ActionType) => void }>();
    ...
    private invoke(item: { stage: number, callback: (action: ActionType) => void }) {
        item.stage = 1;
