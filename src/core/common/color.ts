...
export function colorToHTMLColorHEX(color: Color): string {
    let f = (x: number) => (0x100 | Math.round(x)).toString(16).substring(1);
...
function xyz_to_lab(x: number, y: number, z: number): [number, number, number] {
    // D65 white point:
    let l = 116 * xyz_to_lab_f(y / D65_Yn) - 16;
    let a = 500 * (xyz_to_lab_f(x / D65_Xn) - xyz_to_lab_f(y / D65_Yn));
    let b = 200 * (xyz_to_lab_f(y / D65_Yn) - xyz_to_lab_f(z / D65_Zn));
    return [l, a, b];
}
...
function lab_to_xyz(l: number, a: number, b: number): [number, number, number] {
    let x = D65_Xn * lab_to_xyz_f((l + 16) / 116 + a / 500);
    let y = D65_Yn * lab_to_xyz_f((l + 16) / 116);
    let z = D65_Zn * lab_to_xyz_f((l + 16) / 116 - b / 200);
    return [x, y, z];
}

function hcl_to_lab(h_degree: number, c: number, l: number): [number, number, number] {
    let h = h_degree / 180 * Math.PI;
...
function xyz_to_sRGB(x: number, y: number, z: number): [number, number, number, boolean] {
    ...
    let clamped = r < 0 || g < 0 || b < 0 || r > 255 || g > 255 || b > 255;
...
function sRGB_to_xyz(r: number, g: number, b: number): [number, number, number] {
    let Rl = sRGB_to_xyz_f(r / 255);
    let Gl = sRGB_to_xyz_f(g / 255);
    let Bl = sRGB_to_xyz_f(b / 255);
...
function xyz_to_AppleP3(x: number, y: number, z: number): [number, number, number, boolean] {
    let Rl = x * 2.725394 + y * -1.018003 + z * -0.440163;
    let Gl = x * -0.795168 + y * 1.689732 + z * 0.022647;
    let Bl = x * 0.041242 + y * -0.087639 + z * 1.100929;
...
function AppleP3_to_xyz(r: number, g: number, b: number): [number, number, number] {
    let Rl = AppleP3_to_xyz_f(r / 255);
    let Gl = AppleP3_to_xyz_f(g / 255);
    let Bl = AppleP3_to_xyz_f(b / 255);
    let x = 0.445170 * Rl + 0.277134 * Gl + 0.172283 * Bl;
    let y = 0.209492 * Rl + 0.721595 * Gl + 0.068913 * Bl;
    let z = 0.000000 * Rl + 0.047061 * Gl + 0.907355 * Bl;
    return [x * 100, y * 100, z * 100];
}

export type ColorConverter = (a: number, b: number, c: number) => [number, number, number] | [number, number, number, boolean];
let colorspaces = new Map<string, { fromXYZ: ColorConverter, toXYZ: ColorConverter }>();
let converter_cache = new Map<string, ColorConverter>();
...
export function getColorConverter(from: string, to: string): ColorConverter {
    ...
    if (from == to) {
        ...
        let f1 = colorspaces.get(from).toXYZ;
        let f2 = colorspaces.get(to).fromXYZ;
...
export function interpolateColor(from: Color, to: Color, colorspace: string = "lab"): ColorInterpolation {
    let color_to_space = getColorConverter("sRGB", colorspace);
    let space_to_color = getColorConverter(colorspace, "sRGB");
    let c1 = color_to_space(from.r, from.g, from.b);
    let c2 = color_to_space(to.r, to.g, to.b);
    return (t: number) => {
        t = Math.min(1, Math.max(0, t));
        let [r, g, b] = space_to_color(
...
export function interpolateColors(colors: Color[], colorspace: string = "lab"): ColorInterpolation {
    let color_to_space = getColorConverter("sRGB", colorspace);
    let space_to_color = getColorConverter(colorspace, "sRGB");
    let cs = colors.map(x => color_to_space(x.r, x.g, x.b));
    return (t: number) => {
        t = Math.min(1, Math.max(0, t));
        let pos = t * (colors.length - 1);
