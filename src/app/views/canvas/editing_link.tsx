...
import { Point, ZoomInfo, Geometry, Specification, Prototypes, Dataset, zipArray, zip, getById, getByName, indexOf, uniqueID, Graphics, getIndexById } from "../../../core";
import { classNames } from "../../utils";
import { SnappableGuide } from "./snapping";
import { renderTransform, renderGraphicalElementSVG, renderSVGPath } from "../../renderer";
...
export class EditingLink extends React.Component<EditingLinkProps, EditingLinkState> {
    refs: {
    ...
    public componentDidMount() {
        ...
        this.hammer.on("tap panend", (e) => {
            let pageX = e.center.x;
            let pageY = e.center.y;

            let markInfo = this.getMarkAtPoint(pageX, pageY);
            if (markInfo) {
                ...
                if (markInfo.mode == "begin") {
                    new Actions.SetObjectProperty(this.props.link, "anchor1", null, anchor).dispatch(this.props.store.dispatcher);
                } else {
                    new Actions.SetObjectProperty(this.props.link, "anchor2", null, anchor).dispatch(this.props.store.dispatcher);
                }
            } else {
    ...
    private renderAnchor(coordinateSystem: Graphics.CoordinateSystem, dx: number, dy: number, anchor: Prototypes.LinkAnchor.Description) {
        let transformPoint = (point: Point) => {
            let x = point.x + dx;
            let y = point.y + dy;
        ...
        if (anchor.points.length == 2) {
            ...
            Prototypes.Links.LinksClass.BandPath(path, {
                points: anchor.points.map(p => { return { x: p.x + dx, y: p.y + dy, direction: p.direction } }),
            ...
            let d = renderSVGPath(path.path.cmds);
            return (
                <g transform={transform} >
                    <path d={d} className="element-ghost-stroke" vectorEffect="non-scaling-stroke" />
                    <path d={d} className="element-stroke" vectorEffect="non-scaling-stroke" />
                </g>
            );
            ...
            let p = transformPoint(anchor.points[0]);
            return (
                <g>
    ...
    private renderMarkPlaceholders() {
        let manager = this.props.store.chartManager;

        // Get the two glyphs
        let props = this.props.link.properties as Prototypes.Links.LinksProperties;
        let lineMode: string = props.linkType;
        let glyphs: { glyph: Specification.Glyph, glyphState: Specification.GlyphState, coordinateSystem: Graphics.CoordinateSystem }[] = [];
        switch (this.props.link.classID) {
            case "links.through": {
                let plotSegmentClass = manager.getClassById(props.linkThrough.plotSegment) as Prototypes.PlotSegments.PlotSegmentClass;
                let coordinateSystem = plotSegmentClass.getCoordinateSystem();
                let facets = Prototypes.Links.facetRows(
                    manager.dataflow.getTable(plotSegmentClass.object.table),
                    plotSegmentClass.state.dataRowIndices,
                    props.linkThrough.facetExpressions.map(x => manager.dataflow.cache.parse(x))
                );
                let glyph = getById(manager.chart.glyphs, plotSegmentClass.object.glyph);
                let rowToMarkState = new Map<number, Specification.GlyphState>();
                for (let i = 0; i < plotSegmentClass.state.dataRowIndices.length; i++) {
                    rowToMarkState.set(plotSegmentClass.state.dataRowIndices[i], plotSegmentClass.state.glyphs[i]);
                }
                let firstNonEmptyFacet = 0;
                for (; firstNonEmptyFacet < facets.length; firstNonEmptyFacet++) {
                    if (facets[firstNonEmptyFacet].length >= 2) break;
                }
                if (firstNonEmptyFacet < facets.length) {
                    glyphs = [
                        { glyph: glyph, glyphState: rowToMarkState.get(facets[firstNonEmptyFacet][0]), coordinateSystem },
                        { glyph: glyph, glyphState: rowToMarkState.get(facets[firstNonEmptyFacet][1]), coordinateSystem }
                    ];
                }
            } break;
            case "links.between": {
                let plotSegmentClasses = props.linkBetween.plotSegments.map(x => manager.getClassById(x) as Prototypes.PlotSegments.PlotSegmentClass);
                let glyphObjects = plotSegmentClasses.map(x => getById(manager.chart.glyphs, x.object.glyph));
                glyphs = [
                    { glyph: glyphObjects[0], glyphState: plotSegmentClasses[0].state.glyphs[0], coordinateSystem: plotSegmentClasses[0].getCoordinateSystem() },
                    { glyph: glyphObjects[1], glyphState: plotSegmentClasses[1].state.glyphs[0], coordinateSystem: plotSegmentClasses[1].getCoordinateSystem() }
                ];
            } break;
            case "links.table": {
                let plotSegmentClasses = props.linkTable.plotSegments.map(x => manager.getClassById(x) as Prototypes.PlotSegments.PlotSegmentClass);
                let glyphObjects = plotSegmentClasses.map(x => getById(manager.chart.glyphs, x.object.glyph));
                let linkTable = this.props.store.chartManager.dataflow.getTable(props.linkTable.table);
                let tables = plotSegmentClasses.map((plotSegmentClass) => {
                    let table = this.props.store.chartManager.dataflow.getTable(plotSegmentClass.object.table);
                    let id2RowGlyphIndex = new Map<string, [number, number]>();
                    for (let i = 0; i < plotSegmentClass.state.dataRowIndices.length; i++) {
                        let rowIndex = plotSegmentClass.state.dataRowIndices[i];
                ...
                let rowItem = linkTable.getRow(0);
                ...
                glyphs = [
                    { glyph: glyphObjects[0], glyphState: plotSegmentClasses[0].state.glyphs[i0], coordinateSystem: plotSegmentClasses[0].getCoordinateSystem() },
                    { glyph: glyphObjects[1], glyphState: plotSegmentClasses[1].state.glyphs[i1], coordinateSystem: plotSegmentClasses[1].getCoordinateSystem() }
                ];
            } break;
        }

        // Render mark anchor candidates
        let elements = glyphs.map(({ glyph, glyphState, coordinateSystem }, glyphIndex) => {
            ...
            let marks = glyph.marks.map((element, elementIndex) => {
                if (glyph.marks.length > 1 && element.classID == "mark.anchor") return null;
                let markClass = manager.getMarkClass(glyphState.marks[elementIndex]);
                let mode: "begin" | "end" = glyphIndex == 0 ? "begin" : "end";
                let anchors = markClass.getLinkAnchors(mode);
                anchors = anchors.filter(anchor => {
                    if (lineMode == "line") return anchor.points.length == 1;
                    if (lineMode == "band") return anchor.points.length == 2;
                });
                return (
                    <g key={element._id}>
                        {anchors.map((anchor, index) => (
                            <g
                                className="anchor" key={`m${index}`}
                                ref={(g) => {
                                    if (g != null) {
                            ...
                            >{this.renderAnchor(coordinateSystem, offsetX, offsetY, anchor)}</g>
                        ))}
                    </g>
                );
            });
        ...
        let currentAnchors = glyphs.map(({ glyph, glyphState, coordinateSystem }, glyphIndex) => {
            let anchorX = (glyphState.marks[0].attributes["x"] as number);
            let anchorY = (glyphState.marks[0].attributes["y"] as number);
            let offsetX = (glyphState.attributes["x"] as number) - anchorX;
            let offsetY = (glyphState.attributes["y"] as number) - anchorY;
            let anchor = glyphIndex == 0 ? props.anchor1 : props.anchor2;
            let element = anchor[0].x.element;
            let elementState = glyphState.marks[getIndexById(glyph.marks, element)];
            let anchorDescription: Prototypes.LinkAnchor.Description = {
                element: element,
                points: anchor.map(a => {
                    return {
            ...
            return { coordinateSystem: coordinateSystem, offsetX, offsetY, anchor: anchorDescription };
        });
        ...
        if (currentAnchors.length == 2) {
            ...
            let anchor1 = {
                ...
                points: currentAnchors[0].anchor.points.map(p => { return { x: p.x + currentAnchors[0].offsetX, y: p.y + currentAnchors[0].offsetY, direction: p.direction } }),
                curveness: this.props.link.properties.curveness as number
            };
            let anchor2 = {
                coordinateSystem: currentAnchors[1].coordinateSystem,
                points: currentAnchors[1].anchor.points.map(p => { return { x: p.x + currentAnchors[1].offsetX, y: p.y + currentAnchors[1].offsetY, direction: p.direction } }),
                curveness: this.props.link.properties.curveness as number
            };
            Prototypes.Links.LinksClass.LinkPath(path, props.linkType, props.interpolationType, anchor1, anchor2);
            let transform = `translate(${this.props.zoom.centerX},${this.props.zoom.centerY}) scale(${this.props.zoom.scale})`;
            currentLinkElement = (
                <g transform={transform}>
                    <path d={renderSVGPath(path.path.cmds)} className={`link-hint-${props.linkType}`} />
                </g>
            );
        }
        return (
            <g>
                {currentLinkElement}
                {elements}
                {currentAnchors.map(({ coordinateSystem, offsetX, offsetY, anchor }, index) => (
