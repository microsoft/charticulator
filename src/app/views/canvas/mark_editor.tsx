...
import { Specification, Prototypes, zipArray, zip, Solver, Point, Geometry, ZoomInfo, indexOf } from "../../../core";

import * as globals from "../../globals";
import { ChartStore, GlyphStore, Selection, MarkSelection, GlyphSelection } from "../../stores";
...
import { MarkSnappingSession, MarkSnappableGuide, MoveSnappingSession } from "./snapping";
...
export class MarkEditorView extends React.Component<MarkEditorViewProps, MarkEditorViewState> {
    ...
    public componentDidMount() {
        this.subs.push(this.props.store.addListener(ChartStore.EVENT_GRAPHICS, () => this.forceUpdate()));
        this.subs.push(this.props.store.addListener(ChartStore.EVENT_CURRENT_TOOL, () => {
    ...
    public render() {
        let markStores = this.props.store.markStores;
        let marks = [
            { name: "mark.rect", displayName: "Rectangle", icon: "mark/rect", draggable: true },
            { name: "mark.symbol", displayName: "Symbol", icon: "mark/symbol", draggable: true },
            { name: "mark.line", displayName: "Line", icon: "mark/line", draggable: true },
            { name: "mark.text", displayName: "Text", icon: "mark/text", draggable: true },
            { name: "guide-y", displayName: "Guide Y", icon: "guide/x", draggable: false },
            { name: "guide-x", displayName: "Guide X", icon: "guide/y", draggable: false },
            { name: "guide-coordinator-y", displayName: "Guide Y", icon: "guide/coordinator-x", draggable: false },
            { name: "guide-coordinator-x", displayName: "Guide X", icon: "guide/coordinator-y", draggable: false }
        ]
        return (
            <div className="mark-editor-view">
                {
                    markStores.map(markStore => {
                        return (
                            <SingleMarkView key={`m${markStore._id}`}
                                parent={this}
...
export class SingleMarkView extends React.Component<SingleMarkViewProps, SingleMarkViewState> implements Droppable {
    refs: {
        canvas: SVGElement;
    ...
    public getFitViewZoom(width: number, height: number) {
        let markState = this.props.store.markState;
        if (!markState) return null;
        ...
        for (let elementState of this.props.store.markState.marks) {
            let cls = this.props.store.parent.chartManager.getMarkClass(elementState);
            let bbox = cls.getBoundingBox();
            if (bbox) {
                let xBounds: number[] = [];
                let yBounds: number[] = [];
                switch (bbox.type) {
                    case "anchored-rectangle": {
                        let bboxRect = bbox as Prototypes.BoundingBox.AnchoredRectangle;
                        let cos = Math.cos(bboxRect.rotation / 180 * Math.PI);
                        let sin = Math.sin(bboxRect.rotation / 180 * Math.PI);
                        xBounds = [
                            bboxRect.anchorX + bboxRect.cx + bboxRect.width / 2 * cos + bboxRect.height / 2 * sin,
                            bboxRect.anchorX + bboxRect.cx - bboxRect.width / 2 * cos + bboxRect.height / 2 * sin,
                            bboxRect.anchorX + bboxRect.cx + bboxRect.width / 2 * cos - bboxRect.height / 2 * sin,
                            bboxRect.anchorX + bboxRect.cx - bboxRect.width / 2 * cos - bboxRect.height / 2 * sin
                        ];
                        yBounds = [
                            bboxRect.anchorY + bboxRect.cy + bboxRect.width / 2 * -sin + bboxRect.height / 2 * cos,
                            bboxRect.anchorY + bboxRect.cy - bboxRect.width / 2 * -sin + bboxRect.height / 2 * cos,
                            bboxRect.anchorY + bboxRect.cy + bboxRect.width / 2 * -sin - bboxRect.height / 2 * cos,
                            bboxRect.anchorY + bboxRect.cy - bboxRect.width / 2 * -sin - bboxRect.height / 2 * cos
                        ];
                    ...
                    case "rectangle": {
                        let bboxRect = bbox as Prototypes.BoundingBox.Rectangle;
                        xBounds = [
                    ...
                    case "circle": {
                        let bboxCircle = bbox as Prototypes.BoundingBox.Circle;
                        xBounds = [bboxCircle.cx - bboxCircle.radius, bboxCircle.cx + bboxCircle.radius];
                        yBounds = [bboxCircle.cy - bboxCircle.radius, bboxCircle.cy + bboxCircle.radius];
                    }
                }
        ...
        let cy = (y1 + y2) / 2;
        let overshoot = 0.4;
        let scale1 = width / (1 + Math.abs(x2 - x1) * (1 + overshoot));
        let scale2 = height / (1 + Math.abs(y2 - y1) * (1 + overshoot));
        let scale = Math.min(scale1, scale2);
        let zoom = {
            centerX: width / 2 - cx * scale, centerY: height / 2 + cy * scale,
            scale: scale
        } as ZoomInfo;
        return zoom;
    }
    ...
    public doAutoFit() {
        let newZoom = this.getFitViewZoom(this.props.width, this.props.height);
        if (!newZoom) return;
        let isFocusing = this.props.store.parent.currentSelection instanceof MarkSelection;
    ...
    public onDragEnter(ctx: DragContext) {
        ...
        if (data instanceof DragData.ObjectType) {
            if (Prototypes.isType(data.classID, "mark") || Prototypes.isType(data.classID, "guide")) {
                this.setState({
                ...
                ctx.onDrop((point) => {
                    ...
                    new Actions.AddMarkToGlyph(this.props.store.mark, data.classID, Geometry.unapplyZoom(this.state.zoom, point)).dispatch(this.props.store.parent.dispatcher);
                });
                return true;
            }
        }
    ...
    public componentDidMount() {
        ...
        this.hammer.on("tap", () => {
            new Actions.SelectGlyph(this.props.store.mark).dispatch(this.props.store.dispatcher);
        });
        ...
        this.tokens.push(this.props.store.addListener(GlyphStore.EVENT_STATE, () => {
            this.doAutoFit();
            this.forceUpdate()
        }));
        this.tokens.push(this.props.store.parent.addListener(ChartStore.EVENT_SELECTION, () => {
            this.setState({
                currentSelection: this.props.store.parent.currentSelection
            })
        }));
        this.tokens.push(globals.dragController.addListener("sessionstart", () => {
            ...
            if (session && session.data instanceof DragData.ObjectType) {
                if (Prototypes.isType(session.data.classID, "mark") || Prototypes.isType(session.data.classID, "guide")) {
                    this.setState({
                        showIndicator: true
                    });
                }
    ...
    public renderElement(element: Specification.Element, elementState: Specification.MarkState) {
        let elementClass = this.props.store.parent.chartManager.getMarkClass(elementState);
        let graphics = elementClass.getGraphics(new Graphics.CartesianCoordinates, { x: 0, y: 0 });
        if (!graphics) return null;
        return renderGraphicalElementSVG(graphics);
    }

    public renderDropIndicator() {
        if (!this.state.showIndicator) return null;
        return (
            <rect x={0} y={0} width={this.props.width} height={this.props.height}
                className={classNames("drop-indicator", ["active", this.state.showIndicatorActive])}
            />
        );
    }
    ...
    public getSnappingGuides(): MarkSnappableGuide[] {
        ...
        guides = this.props.store.parent.chartManager.getGlyphClass(this.props.store.markState).getAlignmentGuides().map(g => { return { element: null, guide: g }; });
        for (let [element, elementState] of zip(this.props.store.mark.marks, this.props.store.markState.marks)) {
            let elementClass = this.props.store.parent.chartManager.getMarkClass(elementState);
            guides = guides.concat(elementClass.getSnappingGuides().map(g => { return { element: element, guide: g }; }));
        }
        return guides;
    }
    ...
    public renderMarkHandles() {
        let markClass = this.props.store.parent.chartManager.getGlyphClass(this.props.store.markState);
        let handles = markClass.getHandles();
                ...
                <HandlesView
                    ...
                    onDragStart={(bound, ctx) => {
                        ...
                        ctx.onEnd((e) => {
                            ...
                            if (updates) {
                                new Actions.UpdateGlyphAttribute(this.props.store.mark, updates).dispatch(this.props.store.dispatcher);
                            }
                        });
                    }}
                />
    ...
    public renderAnchorHandles() {
        return zipArray(this.props.store.mark.marks, this.props.store.markState.marks)
            .filter(x => x[0].classID == "mark.anchor")
            .map(([element, elementState], idx) => {
                let elementClass = this.props.store.parent.chartManager.getMarkClass(elementState);
                let bounds = elementClass.getHandles();
                return (
                    <HandlesView
                        key={`m${element._id}`}
                        handles={bounds}
                        ...
                        onDragStart={(bound, ctx) => {
                            ...
                            let session = new MarkSnappingSession(guides, this.props.store.mark, element, elementState, bound, 10 / this.state.zoom.scale);
                            ctx.onDrag((e) => {
                                session.handleDrag(e);
                                this.setState({
    ...
    public renderElementHandles() {
        return zipArray(this.props.store.mark.marks, this.props.store.markState.marks)
            .filter(x => x[0].classID != "mark.anchor")
            .sort((a, b) => {
                let aSelected = this.state.currentSelection instanceof MarkSelection && this.state.currentSelection.mark == a[0];
                let bSelected = this.state.currentSelection instanceof MarkSelection && this.state.currentSelection.mark == b[0];
                if (aSelected) return +1;
                if (bSelected) return -1;
                return this.props.store.mark.marks.indexOf(a[0]) - this.props.store.mark.marks.indexOf(b[0]);
            })
            .map(([element, elementState]) => {
                let elementClass = this.props.store.parent.chartManager.getMarkClass(elementState);
                let shouldRenderHandles = this.state.currentSelection instanceof MarkSelection && this.state.currentSelection.mark == element;
                    ...
                    if (bbox) {
                        return (
                            <BoundingBoxView
                                ...
                                boundingBox={bbox}
                                onClick={() => {
                                    new Actions.SelectMark(this.props.store.mark, element).dispatch(this.props.store.dispatcher);
                                }}
                            />
                        );
                    }
                    ...
                    <g key={`m${element._id}`}>
                        ...
                        <HandlesView
                            ...
                            isAttributeSnapped={(attribute) => {
                                if (element.mappings[attribute] != null) return true;
                                for (let constraint of this.props.store.mark.constraints) {
                                    if (constraint.type == "snap") {
                                        if (constraint.attributes["element"] == element._id && constraint.attributes["attribute"] == attribute) return true;
                                        if (constraint.attributes["targetElement"] == element._id && constraint.attributes["targetAttribute"] == attribute) return true;
                                    }
                                }
                                return false;
                            }}
    ...
    public renderDropZoneForElement(data: any, element: Specification.Element, state: Specification.MarkState) {
        let cls = this.props.store.parent.chartManager.getMarkClass(state);
        return cls.getDropZones().map((zone: Prototypes.DropZones.Description, idx) => {
            if (zone.accept) {
                if (zone.accept.table) {
                    if (data.table.name != zone.accept.table) return null;
                }
                if (zone.accept.kind) {
                    if (data.metadata.kind != zone.accept.kind) return null;
                }
            }
            return (
                <DropZoneView
                        ...
                        if (data instanceof DragData.DataExpression) {
                            ...
                            if (zone.dropAction.scaleInference) {
                                return (point: Point, modifiers: DragModifiers) => {
                                    if (!zone.dropAction.scaleInference.hints) zone.dropAction.scaleInference.hints = {};
                                    zone.dropAction.scaleInference.hints.newScale = modifiers.shiftKey;
    ...
    public renderSnappingGuidesLabels() {
        let allLabels: Prototypes.SnappingGuides.Description[] = [];
        for (let [element, elementState] of zip(this.props.store.mark.marks, this.props.store.markState.marks)) {
            let elementClass = this.props.store.parent.chartManager.getMarkClass(elementState);
            let guides = elementClass.getSnappingGuides();
        ...
        return (
            <g>
                {allLabels.map((guide: Prototypes.SnappingGuides.Label, i: number) => {
                    let x = guide.x * this.state.zoom.scale + this.state.zoom.centerX;
                    let y = -guide.y * this.state.zoom.scale + this.state.zoom.centerY;
                    return (
                        <g transform={`translate(${x},${y})`} className="snapping-guide-label">
                            <circle cx={0} cy={0} r={2} />
                            <text x={5} y={5} transform={`rotate(45)`}>{guide.text}</text>
                        </g>
                    );
                })}
            </g>
    ...
    public renderMarkGuides() {
        let markClass = this.props.store.parent.chartManager.getGlyphClass(this.props.store.markState);
        let markGuides = markClass.getAlignmentGuides();
        return markGuides.map((theGuide, idx) => {
            if (theGuide.type == "x") {
                let guide = theGuide as Prototypes.SnappingGuides.Axis;
                return <line className="mark-guide" key={`k${idx}`} x1={guide.value * this.state.zoom.scale + this.state.zoom.centerX} x2={guide.value * this.state.zoom.scale + this.state.zoom.centerX} y1={0} y2={this.props.height} />
            }
            if (theGuide.type == "y") {
                let guide = theGuide as Prototypes.SnappingGuides.Axis;
                return <line className="mark-guide" key={`k${idx}`} y1={-guide.value * this.state.zoom.scale + this.state.zoom.centerY} y2={-guide.value * this.state.zoom.scale + this.state.zoom.centerY} x1={0} x2={this.props.width} />
            }
        });
    }

    public renderAnchor() {
        let { mark, markState } = this.props.store;
        let anchorIndex = indexOf(mark.marks, x => x.classID == "mark.anchor");
        let pt = {
        ...
        return (
            <path d={`M${pt.x - 5},${pt.y}L${pt.x},${pt.y - 5}L${pt.x + 5},${pt.y}L${pt.x},${pt.y + 5}Z`} className="mark-anchor" />
        )
    }

    public renderCreatingComponent() {
        let currentCreation = this.props.parent.getCurrentCreation();
        if (currentCreation == null) return null;

        let metadata = Prototypes.ObjectClasses.GetMetadata(currentCreation);
        if (metadata && metadata.creatingInteraction) {
            let classID = currentCreation;
            return (
                <CreatingComponentFromCreatingInteraction
                    width={this.props.width} height={this.props.height} zoom={this.state.zoom}
                    ...
                    onCreate={(mappings, attributes) => {
                        ...
                        new Actions.AddMarkToGlyph(
                            this.props.store.mark,
                            classID, { x: 0, y: 0 }, mappings, attributes
                        ).dispatch(this.props.store.dispatcher);
                    }}
                    onCancel={() => {
                        new Actions.SetCurrentTool(null).dispatch(this.props.store.dispatcher);
            ...
            switch (currentCreation) {
                case "mark.rect": {
                    ...
                    onCreate = (x1, y1, x2, y2) => {
                        new Actions.AddMarkToGlyph(this.props.store.mark, "mark.rect", { x: x1[0], y: y1[0] }, { x1: x1, y1: y1, x2: x2, y2: y2 }).dispatch(this.props.store.dispatcher);
                    };
                } break;
                case "mark.line": {
                    mode = "line";
                    onCreate = (x1, y1, x2, y2) => {
                        new Actions.AddMarkToGlyph(this.props.store.mark, "mark.line", { x: x1[0], y: y1[0] }, { x1: x1, y1: y1, x2: x2, y2: y2 }).dispatch(this.props.store.dispatcher);
                    };
                } break;
                case "mark.symbol": {
                    mode = "point";
                    onCreate = (x, y) => {
                        new Actions.AddMarkToGlyph(this.props.store.mark, "mark.symbol", { x: x[0], y: y[0] }, { x: x, y: y }).dispatch(this.props.store.dispatcher);
                    };
                } break;
                case "mark.text": {
                    mode = "point";
                    onCreate = (x, y) => {
                        new Actions.AddMarkToGlyph(this.props.store.mark, "mark.text", { x: x[0], y: y[0] }, { x: x, y: y }).dispatch(this.props.store.dispatcher);
                    };
                } break;
                case "guide-x": {
                    mode = "vline";
            ...
            return (
                <CreatingComponent width={this.props.width} height={this.props.height} zoom={this.state.zoom}
                    mode={mode}
                    key={mode}
                    guides={this.getSnappingGuides()}
            ...
            return (
                <div className="mark-editor-single-view">
                    <div className="mark-view-container">
                        <svg className="canvas-view canvas-view-mark" ref="canvas" x={0} y={0} width={this.props.width} height={this.props.height}>
                            <rect ref="canvasInteraction" className="interaction-handler" x={0} y={0} width={this.props.width} height={this.props.height} />
                        </svg>
                    </div>
                </div>
            );
                ...
                <div className="mark-view-container">
                    <svg className="canvas-view canvas-view-mark" ref="canvas" x={0} y={0} width={this.props.width} height={this.props.height}>
                        ...
                        <g ref="zoomable" transform={transform} className="graphics">
                            {zipArray(mark.marks, markState.marks).map(([elements, elementState]) => {
                                return <g key={`m${elements._id}`}>{this.renderElement(elements, elementState)}</g>;
                            })}
                        </g>
                        {/* {this.renderAnchor()} */}
                        ...
                        <g>{this.state.dataForDropZones ? zipArray(mark.marks, markState.marks).map(([elements, elementState]) => {
                            return <g key={`m${elements._id}`}>{this.renderDropZoneForElement(this.state.dataForDropZones, elements, elementState)}</g>;
                        }) : null}</g>
                    ...
                    <div className="canvas-controls">
                        <Button icon="general/zoom-in" onClick={() => {
                            ...
                            let fixPoint = Geometry.unapplyZoom(this.state.zoom, { x: this.props.width / 2, y: this.props.height / 2 });
