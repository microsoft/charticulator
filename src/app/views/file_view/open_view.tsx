                ...
                return (
                    <ul className="chart-list">
                        {this.state.chartList.map(chart => {
                            return (
                                <li key={chart.id} onClick={() => {
                                    store.backendOpenChart(chart.id).then(() => {
                                        this.props.onClose();
                                    });
                                }}>
                                    <div className="thumbnail"><img src={chart.metadata["thumbnail"] as string} /></div>
                                    <div className="description">
                                        <div className="name" onClick={(e) => e.stopPropagation()}>
                                            <EditableTextView text={chart.metadata.name} onEdit={(newText) => {
                                                backend.get(chart.id).then((chart) => {
                                                    chart.metadata.name = newText;
                                                    backend.put(chart.id, chart.data, chart.metadata).then(() => {
                                                        this.updateChartList();
                                                    });
                                                });
                                            }} />
                                        ...
                                        <div className="footer">
                                            <div className="metadata">{new Date(chart.metadata.timeCreated).toLocaleString()}</div>
                                            <div className="actions">
                                                <ButtonFlat url={R.getSVGIcon("toolbar/trash")} title="Delete this chart" stopPropagation onClick={() => {
                                                ...
                                                <ButtonFlat url={R.getSVGIcon("toolbar/copy")} title="Copy this chart" stopPropagation onClick={() => {
                                                    backend.get(chart.id).then((chart) => {
                                                        backend.create("chart", chart.data, chart.metadata).then(() => {
                                                            this.updateChartList();
                                                        });
                                                    });
                                                }} />
                                                <ButtonFlat url={R.getSVGIcon("toolbar/export")} title="Export this chart" stopPropagation onClick={() => {
                                                    backend.get(chart.id).then((chart) => {
                                                        let blob = new Blob([
                                                            JSON.stringify(chart.data, null, 2)
                                                        ]);
                                                        FileSaver.saveAs(blob, chart.metadata.name.replace(/[^0-9a-zA-Z\ \.\-\_]+/g, "_") + ".json");
                                                    });
                                                }} />
                                            </div>
                                        </div>
