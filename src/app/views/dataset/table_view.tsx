            ...
            <table className="charticulator__dataset-table-view">
                ...
                <tbody>
                    {table.rows.slice(0, maxRows).map(r => (
                        <tr key={r._id}>
                            {table.columns.map(c => <td key={c.name}>{r[c.name].toString()}</td>)}
                        </tr>
                    ))}
                    {table.rows.length > maxRows ? <tr>{table.columns.map((c, i) => i == 0 ? <td key={i}>({table.rows.length - maxRows} more rows)</td> : <td key={i}>...</td>)}</tr> : null}
                </tbody>
            </table>
        )
    }
