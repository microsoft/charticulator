...
export interface DataFieldSelectorProps {
    ...
    /** Set a default value */
    defaultValue?: { table: string, lambdaExpression?: string, expression?: string };
...
export class DataFieldSelector extends React.Component<DataFieldSelectorProps, DataFieldSelectorState> {
    constructor(props: DataFieldSelectorProps) {
        super(props);

        if (this.props.defaultValue) {
            let fs = this.getFields().filter(x => {
                if (this.props.defaultValue.table != null && x.table != this.props.defaultValue.table) return false;
    ...
    private getFields() {
        let store = this.props.datasetStore;
        let columns = store.getTables()[0].columns;
        let candidates = columns.map(c => {
            return {
                table: store.getTables()[0].name,
                columnName: c.name,
                expression: Expression.variable(c.name).toString(),
                lambdaExpression: Expression.lambda(["x"], Expression.fields(Expression.variable("x"), c.name)).toString(),
                type: c.type,
                metadata: c.metadata,
            };
        });
        ...
        if (this.props.kinds) {
            candidates = candidates.filter(x => x.metadata != null && this.props.kinds.indexOf(x.metadata.kind) >= 0);
        }
        if (this.props.types) {
            candidates = candidates.filter(x => x.metadata != null && this.props.types.indexOf(x.type) >= 0);
        }
        return candidates;
    }
        ...
        return (
            <div className="charticulator__data-field-selector">
                {this.props.nullDescription ? (
                    <div
                        className={classNames("el-item", "is-null", ["is-active", !this.props.nullNotHighlightable && this.state.currentSelection == null])}
                ...
                {fields.map(f => {
                    return (
                        <div
                            ...
                            className={classNames("el-item", ["is-active", this.isValueEqual(this.state.currentSelection, f)])}
