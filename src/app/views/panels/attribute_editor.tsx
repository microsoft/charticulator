...
import { Droppable, DragContext, PopupView, DragModifiers } from "../../controllers";
import { Prototypes, Specification, Color, getById, Point } from "../../../core";
...
export interface AttributeEditorProps {
    ...
    onMapData: (name: string, data: DragData.DataExpression, newScale: boolean) => void;
...
export class AttributeEditor extends React.Component<AttributeEditorProps, AttributeEditorState> {
            ...
            <ul className="attribute-list">
                {names.map(name => (
                    <AttributeEditorItem key={name}
                        ...
                        onMapData={(data, newScale) => this.props.onMapData(name, data, newScale)}
    ...
    public render() {
        let attrs = this.props.attributeNames.filter(x => this.props.attributeDescriptions[x].mode != "positional");
        let categories = new Set();
        for (let attr of attrs) {
            let category = this.props.attributeDescriptions[attr].category;
            if (category) {
                ...
                {categoryOrder.map(c => {
                    return (
                        <section key={c[0]}>
                            ...
                            {this.renderItems(attrs.filter(x => this.props.attributeDescriptions[x].category == c[0]))}
                        </section>
                    );
                })}
            </ul>
...
export class AttributeEditorItem extends React.Component<AttributeEditorItemProps, AttributeEditorItemState> implements Droppable {
    refs: {
    ...
    public renderMapping(item: Specification.Mapping) {
        if (item == null || item.type == "value") {
            let value = item as Specification.ValueMapping;
            return (
                <ValueMappingEditor
        ...
        switch (item.type) {
            ...
            case "scale": {
                let scale = item as Specification.ScaleMapping;
                if (scale.scale) {
                    let scaleObject = getById(this.props.chart.scales, scale.scale);
                    if (scaleObject) {
                        return (
                            <span className="mapping-scale" onClick={() => {
                                globals.popupController.popupAt(
                                    (context) => (
                                        <PopupView context={context}>
                                            <ScaleEditor scale={scaleObject} scaleMapping={scale} store={this.props.store} />
                                        </PopupView>
                                    ),
                                    { anchor: this.refs.content }
                                );
                            }}>
                                <span className="mapping-scale-scale left">{scaleObject.properties.name}</span>
    ...
    public render() {
        return (
            <li className={classNames("attribute-item", ["active", this.state.draggingOver != null])} ref="container">
                <label>{this.props.attributeDescription.displayName || this.props.attributeName}</label>
                <span className="content" ref="content">
                    {this.renderMapping(this.props.mapping)}
                </span>
                <span className="buttons">
                    {this.props.mapping && this.props.mapping.type == "scale" && (this.props.mapping as Specification.ScaleMapping).scale ? (
                        <span className="button" title="Add legend for this scale" onClick={() => {
                            new Actions.ToggleLegendForScale((this.props.mapping as Specification.ScaleMapping).scale).dispatch(this.props.store.dispatcher);
                        }}><SVGImageIcon url={R.getSVGIcon("legend/legend")} /></span>
                    ) : null}
                    {this.props.mapping ? <span className="button" title="Remove" onClick={() => this.props.onEdit(null)}><SVGImageIcon url={R.getSVGIcon("general/eraser")} /></span> : null}
                </span>
            </li>
        );
    }
