...
import { Prototypes, Specification, Color, ColorGradient, getById, prettyNumber, deepClone } from "../../../../core";
import { ColorPicker, GradientPicker, ButtonFlat, DropdownButton, SVGImageIcon, GradientView, ButtonFlatPanel } from "../../../components";
...
export class ValueMappingEditor extends React.Component<ValueMappingEditorProps, ValueMappingEditorState> {
    refs: {
                    ...
                    globals.popupController.popupAt(
                        (context) => (
                            <PopupView context={context}>
                                <ColorPicker
                                    defaultValue={this.props.mapping ? this.props.mapping.value as Color : null}
                                    onPick={(color) => {
                                        this.props.onEdit({ type: "value", value: color } as Specification.ValueMapping)
                                    }}
                                />
                            </PopupView>
                        ),
                ...
                case "color-gradient": {
                    globals.popupController.popupAt(
                        (context) => (
                            <PopupView context={context}>
                                <GradientPicker
                                    defaultValue={this.props.mapping ? this.props.mapping.value as any as ColorGradient : null}
                                    onPick={(gradient: ColorGradient) => {
                                        this.props.onEdit({ type: "value", value: gradient as Specification.Types.ColorGradient } as Specification.ValueMapping)
                                    }}
                                />
                            </PopupView>
                        ),
    ...
    public render() {
        if (!this.state.editing) {
            return (
                <span className="value-mapping-editor" ref="content" tabIndex={0}
                    onClick={this._startEditing}
                    onFocus={this._startEditingFocused}
                >
                    {this.props.defaultValue != null ? <ValueView type={this.props.type} value={this.props.defaultValue} /> : <span className="type-unknown">(none)</span>}
                </span>
            );
        } else {
            return (
                <span className="value-mapping-editor" ref="content" onClick={this._startEditing}>
                    <InlineEditorView
                        type={this.props.type} value={this.props.mapping != null ? this.props.mapping.value : this.props.defaultValue}
                        description={this.props.description}
                        onEdit={(newValue, final) => {
                            this.props.onEdit({ type: "value", value: newValue } as Specification.ValueMapping);
                        }}
                        onCancel={() => {
                            this.setState({
                                editing: this.props.mapping != null
...
export function ValueView(props: { type: string, value: Specification.AttributeValue }) {
    switch (props.type) {
        case "number": {
            let value = props.value as number;
            let str = prettyNumber(value, 8);
            return (<span className="type-number">{str}</span>);
        }
        case "boolean": {
            let value = props.value as boolean;
            return (
                <span className="type-boolean">
                    {value ? "Yes" : "No"}
                </span>
            );
        }
        ...
        case "color": {
            let value = props.value as Color;
            let rgb = `rgb(${value.r.toFixed(0)},${value.g.toFixed(0)},${value.b.toFixed(0)})`;
            return (<span className="type-color" style={{ backgroundColor: rgb }} />);
        }
        case "axis-data-binding": {
            let value = props.value as Specification.Types.AxisDataBinding;
            if (value.column) {
        ...
        case "color-gradient": {
            let value = props.value as Specification.Types.ColorGradient;
            return (<span className="type-color-gradient"><GradientView gradient={value} /></span>);
        }
        case "text-alignment": {
            let value = props.value as Specification.Types.TextAlignment;
            let xIcon = R.getSVGIcon({ "left": "align/left", "middle": "align/x-middle", "right": "align/right" }[value.x]);
            let yIcon = R.getSVGIcon({ "top": "align/top", "middle": "align/y-middle", "bottom": "align/bottom" }[value.y]);
            return (
...
export class InlineEditorViewNumber extends React.Component<InlineEditorViewProps, {}> {
    ...
    public render() {
        ...
        if (props.description.defaultRange) {
            let range = props.description.defaultRange as [number, number];
            return (
                <span className="type-number-editor">
                    {inputElement}
                    <Slider width={100} min={range[0]} max={range[1]} onChange={(v, final) => {
                        if (final) {
...
export function InlineEditorViewBoolean(props: InlineEditorViewProps) {
    ...
    return (
        <span className="type-boolean-editing" >
            <ButtonFlatPanel url={R.getSVGIcon(value ? "checkbox/checked" : "checkbox/empty")} text={value ? "Yes" : "No"} onClick={() => {
...
export function InlineEditorViewColor(props: InlineEditorViewProps) {
    ...
    return (
        <span className="type-color-editing" >
            <span className="type-color" ref={(v) => contentView = v} style={{ backgroundColor: rgb }} onClick={() => {
                globals.popupController.popupAt(
                    (context) => (
                        <PopupView context={context}>
                            <ColorPicker
...
export function InlineEditorViewTextAlignment(props: InlineEditorViewProps) {
    ...
    return (
        <span className="type-text-alignment-editor">
            <span className="editor-row">
                <DropdownButton
                    list={[
                        ...
                        { name: "middle", text: "Middle", url: R.getSVGIcon("align/x-middle") },
                        { name: "right", text: "Right", url: R.getSVGIcon("align/right") }
                    ]}
                    url={R.getSVGIcon({ "left": "align/left", "middle": "align/x-middle", "right": "align/right" }[valueClone.x])}
                    onSelect={(name) => {
                ...
                <DropdownButton
                    list={[
                        ...
                        { name: "middle", text: "Middle", url: R.getSVGIcon("align/y-middle") },
                        { name: "bottom", text: "Bottom", url: R.getSVGIcon("align/bottom") }
                    ]}
                    url={R.getSVGIcon({ "top": "align/top", "middle": "align/y-middle", "bottom": "align/bottom" }[valueClone.y])}
                    onSelect={(name) => {
            ...
            {valueClone.x != "middle" ? <span className="editor-row">x: <InputText defaultValue={prettyNumber(valueClone.xMargin)} onEnter={(newValue) => {
                let num = parseFloat(newValue);
                if (num == num && num != null) {
            ...
            {valueClone.y != "middle" ? <span className="editor-row">y: <InputText defaultValue={prettyNumber(valueClone.yMargin)} onEnter={(newValue) => {
                let num = parseFloat(newValue);
                if (num == num && num != null) {
...
export function InlineEditorViewMapFactory(targetType: string) {
    return function InlineEditorViewMapType(props: InlineEditorViewProps): JSX.Element {
        let value = props.value as { [name: string]: Specification.AttributeValue };
        let valueClone = deepClone(value);
            ...
            <span className="type-map-editor">
                {keys.map((key, idx) => (
                    <span className="editor-row" key={`m${idx}`}>
                        ...
                        <InlineEditorView
                            value={valueClone[key]} type={targetType} description={{ name: key, type: targetType }}
                            onEdit={(value) => {
...
export class InlineEditorView extends React.Component<InlineEditorViewProps, {}> {
    public render() {
        let props = this.props;
        switch (props.type) {
            case "number":
