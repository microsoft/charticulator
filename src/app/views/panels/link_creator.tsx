...
import { ButtonRaised, ToolButton, ButtonFlatPanel, SVGImageIcon } from "../../components";
import { Radio } from "../panels/widgets/controls";
import { classNames } from "../../utils";
import { ReorderListView } from "./object_list_editor";
import { Specification, Prototypes, getById, uniqueID, Graphics, argMax, Point, argMin, Geometry } from "../../../core";
...
export class LinkCreationPanel extends ContextedComponent<LinkCreationPanelProps, LinkCreationPanelState> {
    ...
    private getDefaultState(): LinkCreationPanelState {
        let plotSegments = this.context.store.chartStore.chart.elements as Specification.PlotSegment[];
        plotSegments = plotSegments.filter(x => Prototypes.isType(x.classID, "plot-segment"));
        let selectedPlotSegments = plotSegments.map(x => x._id);
            ...
            <div className="charticulator__link-type-table">
                <div className="el-row">
                    ...
                    <PanelRadioControl
                        ...
                        onChange={(newValue: "line" | "band") => this.setState({ linkType: newValue })}
                        showText
                    />
                </div>
                ...
                {this.state.selectedPlotSegments.length == 1 && this.isLinkDataPresent() ? (
                    <div className="el-row">
                        <h2>Link Mode:</h2>
                        <PanelRadioControl
                ...
                <div className="el-row">
                    <ButtonRaised text="Create Links" onClick={() => {
                        let links = this.getLinkObject();
                        if (links != null) {
                    ...
                    {this.state.errorReport ? <span>{this.state.errorReport}</span> : null}
                </div>
            </div>
        );
    }
    ...
    private getDefaultAnchor(
        ...
        glyph1: Specification.Glyph, glyphState1: Specification.GlyphState,
        glyph2: Specification.Glyph, glyphState2: Specification.GlyphState
    ): {
        ...
        for (let mark of glyphState1.marks) {
            let c = manager.getMarkClass(mark);
            candidates1 = candidates1.concat(c.getLinkAnchors("begin"));
        }
        for (let mark of glyphState2.marks) {
            let c = manager.getMarkClass(mark);
        ...
        let glyphAttributes1 = glyphState1.attributes as Prototypes.Glyphs.RectangleGlyphAttributes;
        let glyphAttributes2 = glyphState2.attributes as Prototypes.Glyphs.RectangleGlyphAttributes;

        let determineRelationship = (a1: number, a2: number, b1: number, b2: number): "before" | "overlap" | "after" => {
        ...
        let xRelationship = determineRelationship(glyphAttributes1.x1, glyphAttributes1.x2, glyphAttributes2.x1, glyphAttributes2.x2);
        let yRelationship = determineRelationship(glyphAttributes1.y1, glyphAttributes1.y2, glyphAttributes2.y1, glyphAttributes2.y2);

        let meanPoint = (points: Point[]): Point => {
            let x = 0, y = 0;
        ...
        let c1: Prototypes.LinkAnchor.Description = null, c2: Prototypes.LinkAnchor.Description = null;
        if (xRelationship == "after") {
            ...
            if (yRelationship == "after") {
                ...
                c1 = candidates1[argMin(candidates1, c => Math.abs(meanPoint(c.points).y))];
                c2 = candidates2[argMin(candidates2, c => Math.abs(meanPoint(c.points).y))];
            }
        }
        ...
        switch (this.state.linkType) {
            case "line": {
                if (c1 == null) {
                    c1 = { element: null, points: [{ xAttribute: "icx", yAttribute: "icy", x: 0, y: 0, direction: { x: 0, y: 0 } }] }
                }
                if (c2 == null) {
                    c2 = { element: null, points: [{ xAttribute: "icx", yAttribute: "icy", x: 0, y: 0, direction: { x: 0, y: 0 } }] }
                }
            } break;
            case "band": {
                if (c1 == null) {
                    c1 = {
                        element: null, points: [
                            { xAttribute: "icx", yAttribute: "iy1", x: 0, y: 0, direction: { x: 1, y: 0 } },
                            { xAttribute: "icx", yAttribute: "iy2", x: 0, y: 0, direction: { x: 1, y: 0 } }
                        ]
                    }
                }
                if (c2 == null) {
        ...
        if (linkMode != "link-table") {
            if (c1.element != null) {
                let element1 = getById(glyph1.marks, c1.element);
        ...
        return {
            ...
            anchor1, anchor2, color, opacity
        };
    }

    public getLinkObject() {
        let manager = this.context.store.chartStore.chartManager;
        ...
        let plotSegmentIDs = this.state.selectedPlotSegments;
        let plotSegmentClasses = plotSegmentIDs.map(x => manager.getClassById(x) as Prototypes.PlotSegments.PlotSegmentClass);
        let glyphs = plotSegmentClasses.map(c => manager.getObjectById(c.object.glyph) as Specification.Glyph);

        switch (this.state.linkMode) {
            case "link-through": {
                // Find the first pair of glyphs
                let plotSegmentClass = plotSegmentClasses[0];
                let glyph = glyphs[0];

                let facetBy = this.groupBySelector ? (this.groupBySelector.value ? [this.groupBySelector.value.expression] : []) : [];
                ...
                let layoutState = plotSegmentClass.state;
                ...
                let { linkType, interpolationType, anchor1, anchor2, color, opacity } = this.getDefaultAnchor(
                    manager,
                    "link-through",
                    plotSegmentClass.getCoordinateSystem(),
                    glyph, rowToMarkState.get(facets[0][0]), glyph, rowToMarkState.get(facets[0][1])
                );

                let links: Specification.Links = {
                    _id: uniqueID(),
            ...
            case "link-between": {
                // Find the first pair of glyphs
                let firstGlyphs = plotSegmentClasses.map(x => x.state.glyphs[0]);
                let { linkType, interpolationType, anchor1, anchor2, color, opacity } = this.getDefaultAnchor(
                    manager,
                    "link-between",
                    new Graphics.CartesianCoordinates(),
                    glyphs[0], firstGlyphs[0], glyphs[1], firstGlyphs[1]
                );
            ...
            case "link-table": {
                // Find the first pair of glyphs
                let firstGlyphs = plotSegmentClasses[0].state.glyphs;
                let { linkType, interpolationType, anchor1, anchor2, color, opacity } = this.getDefaultAnchor(
                    manager,
                    "link-table",
                    plotSegmentClasses[0].getCoordinateSystem(),
                    glyphs[0], firstGlyphs[0], glyphs[0], firstGlyphs[1]
                );
                ...
                let links: Specification.Links = {
                    ...
                    properties: {
                        ...
                        linkTable: {
                            ...
                            plotSegments: [plotSegmentClasses[0].object._id, plotSegmentClasses[0].object._id]
                        },
                        curveness: 30
                    } as Prototypes.Links.LinksProperties
                };
...
export class PlotSegmentSelector extends ContextedComponent<PlotSegmentSelectorProps, PlotSegmentSelectorState> {
    state: PlotSegmentSelectorState = this.getInitialState();

    private getInitialState(): PlotSegmentSelectorState {
        let plotSegments = this.props.items;
        return {
        ...
        return (
            <div className="charticulator__plot-segment-selector charticulator-panel-list-view is-list">
                <ReorderListView enabled onReorder={(a, b) => {
                    ...
                    let newSelection = this.state.order.filter(x => this.state.selection.indexOf(x) >= 0);
                    this.setState({
                        order: newOrder,
                    ...
                    {this.state.order.map(id => {
                        let item = getById(this.props.items, id);
                        return (
                            <div
                                key={id}
                                className={classNames("el-item", ["is-active", this.state.selection.indexOf(id) >= 0])}
                                onClick={(e) => {
                                    if (e.shiftKey) {
                                        let newSelection = this.state.order.filter(x => x == id || this.state.selection.indexOf(x) >= 0);
                                        this.setState({
                                            selection: newSelection
                                        }, () => this.notify());
                                    } else {
                            ...
                            >
                                <SVGImageIcon url={R.getSVGIcon(Prototypes.ObjectClasses.GetMetadata(item.classID).iconPath)} />
...
export class PanelRadioControl extends React.Component<PanelRadioControlProps, {}> {
    public render() {
        let mainClass = this.props.asList ? "charticulator-panel-list-view" : "charticulator-panel-list-view is-inline"
        return (
            <span className={mainClass}>
                {this.props.options.map((option, index) => {
                    return (
                        <span
                            className={classNames("el-item", ["is-active", this.props.value == option])}
                            key={option}
                            onClick={() => {
                                if (this.props) this.props.onChange(option);
                            }}
                        >
                            {this.props.icons ? <SVGImageIcon url={R.getSVGIcon(this.props.icons[index])} /> : null}
                            {this.props.labels && this.props.showText ? <span className="el-text">{this.props.labels[index]}</span> : null}
                        </span>
                    );
                })}
            </span>
