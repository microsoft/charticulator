...
export class DatasetStore extends BaseStore {
    ...
    public getColumnVector(table: Dataset.Table, columnName: string): Dataset.ValueType[] {
    ...
    public setSelectedRowIndex(table: Dataset.Table, dataRowIndex: number, emit: boolean = true) {
        if (dataRowIndex != null && dataRowIndex >= 0 && dataRowIndex < table.rows.length) {
