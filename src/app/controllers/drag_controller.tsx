...
export class DragContext {
    ...
    public onLeave(f: () => void) { this._onleave = f; }
    public onOver(f: (point: Point, modifiers: DragModifiers) => void) { this._onover = f; }
    public onDrop(f: (point: Point, modifiers: DragModifiers) => void) { this._ondrop = f; }
...
export class DragSession {
    ...
    public handlePan(point: Point, modifiers: DragModifiers) {
        ...
        let withins = new Set<Droppable>();

        while (element != null) {
            let droppable = this.parent.getDroppableFromElement(element);
            if (droppable) {
        ...
        this.states.forEach((context, droppable) => {
            if (!withins.has(droppable)) {
                if (context._state == 1) {
                    if (context._onleave) context._onleave();
                    context._state = 0;
...
export class DragController extends EventEmitter {
    ...
    public registerDraggable(obj: Draggable, rootElement: Element, onTap?: () => void) {
        // Remove any existing stuff
        this.unregisterDraggable(obj);

        // Create hammer object and setup handlers
        let hammer = new Hammer.Manager(rootElement);
        hammer.add(new Hammer.Pan());

        hammer.on("panstart", (e) => {
            try {
                this._dragSession = new DragSession(this, obj, { x: e.center.x, y: e.center.y });
            } catch(e) {
                console.trace(e);
                return;
            }
    ...
    public unregisterDraggable(obj: Draggable) {
        if (this._draggables.has(obj)) {
            let info = this._draggables.get(obj);
            info.hammer.destroy();
...
export class DragStateView extends React.Component<{ controller: DragController }, {}> {
    ...
    public render() {
        let session = this.props.controller.getSession();
        if (!session) return <div></div>;
        let [element, offset] = session.obj.renderDragElement();
        return (
            <div className="drag-state-view" style={{
                position: "fixed",
