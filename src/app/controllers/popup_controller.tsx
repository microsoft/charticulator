...
let popupViewMapping = new WeakMap<HTMLDivElement, PopupContext>();
...
export class PopupController extends EventEmitter {
    ...
    public popupAt(renderElement: (context: PopupContext) => JSX.Element, options: PopupOptions) {
        if (options.alignX == undefined) options.alignX = "start-inner";
        if (options.alignY == undefined) options.alignY = "outer";
        if (!options.parent && options.anchor) {
            options.parent = findParentPopup(options.anchor);
        }
        let context = new PopupContext("#" + (this.currentID++).toString(), renderElement, options);
        if (!options.parent) {
    ...
    public showModal(renderElement: (context: PopupContext) => JSX.Element, options: PopupOptions) {
        let context = new PopupContext("#" + (this.currentID++).toString(), renderElement, options);
        this.reset();
...
export class PopupContainer extends React.Component<PopupViewProps, {}> {
    ...
    public render() {
        if (this.props.controller.currentModal) {
            let modal = this.props.controller.currentModal;
            return (
                <div className="popup-container popup-container-modal" style={{
        ...
        if (popups.length == 0) {
            ...
            return (
                <div className="popup-container" style={{
                    ...
                    {popups.map(popup => {
                        return (
                            <div key={popup.id} ref={(ref) => { if (ref) popupViewMapping.set(ref, popup) }}>
                                {popup.element}
                            </div>
                        );
                    })}
...
export class PopupView extends React.Component<{
    ...
    public render() {
        let popup = this.props.context;
        let position = popup.options.anchor.getBoundingClientRect();
        ...
        switch (popup.options.alignX) {
            case "inner": {
                if ((position.left + position.right) / 2 < window.innerWidth / 2) {
                    ...
                    style.right = (window.innerWidth - (position.left + position.width)) + "px";
                    alignX = "end-inner";
                }
            } break;
            case "outer": {
        ...
        switch (popup.options.alignY) {
            case "inner": {
                if ((position.top + position.bottom) / 2 < window.innerHeight / 2) {
                    ...
                    style.bottom = (window.innerHeight - (position.top + position.height)) + "px";
                    alignY = "end-inner";
                }
            } break;
            case "outer": {
        ...
        return (
            <div className="popup-view-container" style={style}
                onMouseDownCapture={(e) => {
                    newlyCreatedContexts = new WeakSet<PopupContext>();
                }}
                onMouseDown={(e) => {
                    e.stopPropagation();
                    for (let child of this.props.context.children) {
            ...
            >
                <div className={classNames("popup-view",
                    ["popup-x-top-left", alignX == "start-inner" && alignY == "end-outer"],
                    ["popup-x-bottom-left", alignX == "start-inner" && alignY == "start-outer"],
                    ["popup-x-top-right", alignX == "end-inner" && alignY == "end-outer"],
                    ["popup-x-bottom-right", alignX == "end-inner" && alignY == "start-outer"],
                    ["popup-y-top-left", alignX == "start-outer" && alignY == "start-inner"],
                    ["popup-y-top-right", alignX == "end-outer" && alignY == "start-inner"],
                    ["popup-y-bottom-left", alignX == "start-outer" && alignY == "end-inner"],
                    ["popup-y-bottom-right", alignX == "end-outer" && alignY == "end-inner"]
                )}>
                    <ErrorBoundary>
                        {this.props.children}
                    </ErrorBoundary>
...
export class ModalView extends React.Component<{
    ...
    public render() {
        ...
        return (
            <div className={`charticulator__modal-${type}`} onMouseDown={(e) => { e.stopPropagation(); }}>
