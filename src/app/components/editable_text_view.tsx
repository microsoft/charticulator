...
export class EditableTextView extends React.Component<EditableTextViewProps, EditableTextViewState> {
    ...
    public componentDidUpdate(prevProps: EditableTextViewProps, prevState: EditableTextViewState) {
        ...
        if (this.state.editing) {
            return (
                <div className="editable-text-view editable-text-view-editing">
                    <input type="text" ref="input" value={this.state.currentText}
                        onChange={(e) => this.setState({ currentText: this.refs.input.value })}
                        onKeyDown={(e) => {
                            if (e.key == "Enter") {
                                this.confirmEdit();
                            }
                    ...
                    />
                    <ButtonFlat url={R.getSVGIcon("general/confirm")} onClick={this.confirmEdit} stopPropagation />
                </div>
            );
        } else {
            return (
