...
export class SelectionView extends React.Component<SelectionViewProps, SelectionViewState> {
    ...
    public componentDidMount() {
        ...
        this.hammer.on("panstart", (e) => {
            let rect = this.refs.handler.getBoundingClientRect();
            let [x, y] = [e.center.x - rect.left, e.center.y - rect.top];
            currentMarquee = {
                x1: x, y1: y,
                x2: x, y2: y
            };
    ...
    public render() {
        return (
            <g>
                <rect ref="handler" className="interaction-handler" style={{ cursor: "crosshair" }} x={this.props.x} y={this.props.y} width={this.props.width} height={this.props.height} />
                {this.state.marquee ? (
                    <rect
