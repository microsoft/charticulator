...
import { CharticulatorCoreConfig, initialize, Dispatcher, Graphics, Point } from "../core";
...
export class TestApplicationView extends React.Component<{}, { slider1: number, slider2: number }> {
    constructor(props: {}) {
        super(props);
        this.state = {
            slider1: 0,
    ...
    public render() {
        ...
        let dAngle = this.state.slider2;

        let testAngles = (angle1: number, angle2: number) => {
            angle1 += dAngle;
            angle2 += dAngle;
            let r = 40;
            let cx = 50 + (gridIndex % 6) * 100 - 300;
            let cy = 50 + Math.floor(gridIndex / 6) * 100 - 300;
            gridIndex += 1;
        ...
        return (
            <div>
                ...
                <input type="range" min={0} max={1000} value={this.state.slider1} onChange={(e) => {
                    this.setState({ slider1: +e.target.value });
                }} />
                <input type="range" min={0} max={1000} value={this.state.slider2} onChange={(e) => {
...
export class TestApplication {
    public initialize(config: CharticulatorCoreConfig, containerID: string) {
        let rpc = new CharticulatorWorker("./dist/scripts/worker.bundle.js");
        async function workerTest() {
            await rpc.initialize(config);
        }
        workerTest();
        return initialize(config).then(() => {
            ReactDOM.render(<TestApplicationView />, document.getElementById(containerID));
        });
    }
}